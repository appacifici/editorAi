...
class AlertUtility implements IAlertService{
    ...
    public async write(process:string, processName:string, originSite:string, destinationSite:string, articleId?:string): Promise<boolean> {        
    ...
    public setAlert(code: string, alert: any, block:boolean = true ): void {        
        ...
        if( block === true ) {
            msg = '[#---------------------------------------#]';
            this.processes[code]['alert'].push(msg);
        }        
    }
    ...
    public setDebug(code: string, debug: any, block:boolean = true, label?: string | null): void {
        ...
        if( block === true ) {
            msg = '[#---------------------------------------#]';
            this.processes[code]['debug'].push(msg);
        }
    }
    ...
    public setError(code: string, error: any, block:boolean = true): void {           
        ...
        if( block === true ) {
            msg = '[#---------------------------------------#]';
            this.processes[code]['error'].push(msg);
        }

        console.log(this.processes[code]['error']);     
    }

    public formatError(error: Error): string {
        let formattedError = `Error: ${error.message}\nStack: ${error.stack || 'no stack trace available'}`;
        Object.getOwnPropertyNames(error).forEach(key => {
          // Utilizziamo l'asserzione di tipo per dire a TypeScript che 'key' è una chiave di 'Error'
    ...
    public setGeneral(code: string, general: any, block:boolean = true): void {
        ...
        if( block === true ) {
            msg = '[#---------------------------------------#]';
            this.processes[code]['general'].push(msg);
        }
    }
    ...
    public setCallData(code: string, callData: any, block:boolean = true): void {
        ...
        if( block === true ) {
            msg = '[#---------------------------------------#]';
            this.processes[code]['callData'].push(msg);
        }
    }
    ...
    public setCallResponse(code: string, callResponse: any, block:boolean = true): void {                
        ...
        if( block === true ) {
            msg = '[#---------------------------------------#]';
            this.processes[code]['callResponse'].push(msg);
        }        
    }
